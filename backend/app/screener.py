...
def status(universe: str = "sp500") -> dict:
    ...
    with _lock:
        ...
        completed_universes = _job.get("completedUniverses", [])
        current_universe_idx = _job.get("currentUniverseIndex", 0)

        # Always return the cached completed results for the selected universe
        ...
        return {
            ...
            "done": cached_data["done"],
            "found": cached_data["found"],
            "results": cached_data["results"],
            "elapsed": None,
            "error": None,
            
            # Diagnostic / background job state
            "jobState": _job["state"],
            "jobUniverse": _job["universe"],
            "jobDone": _job["done"],
            "jobTotal": _job["total"],
            "completedUniverses": completed_universes,
            "currentUniverseIndex": current_universe_idx,
        }
...
def _run_all_sequential(settings: HGSettings, force: bool = False):
    ...
    with _lock:
        ...
        _job["completedUniverses"] = []
        _job["currentUniverseIndex"] = 0
    
    try:
        for i, uni in enumerate(universes):
            with _lock:
                _job["currentUniverseIndex"] = i
            ...
            with _lock:
                _job["completedUniverses"].append(uni)
